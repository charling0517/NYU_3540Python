...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    for s in iot_sensor_points:
        temperatures.append(iot_sensor_points[s])
    #3
    more_temperatures = [a, b, c, d, e]
    for n in more_temperatures:
...
# ------ Place code below here \/ \/ \/ ------
#Show the food in menu 
def menu_items(menu):
    print("The menu includes: ")
    for i in range(len(menu)):
        print(i+1, '. ', menu[i])
        
#Show what food have been bought 
def bought_items(menu, user_input):
    item = menu[user_input-1]
    print(item + " are already bought")
...
def display_menu(menu):
    ...
    user_input = input("Please select items according to the item number (Press 'E' to exit): ")
    num_of_input = 0
    while user_input != 'E':
        if user_input in menu_list:
            bought_items(menu, int(user_input))
        else:
    ...
    print("The toal order: " + str(num_of_input))
...
def exercise10(sentence):
    ...
    for s in sentence:
        if s == ' ':
            ...
            reversed = reversed + s.lower()   
        elif s.islower():
            reversed = reversed + s.upper()   
